...
CacheLRU.prototype.get = function(key) {
    if(key) {
        ...
        return {
            key: JSON.parse(this.data[key].toString())
        };
    }
    var obj = {};
    for(let key in this.data){
        obj[key] = JSON.parse(this.data[key].toString());
    }
...
CacheLRU.prototype.put = function(key, value) {
    ...
    refresh(this.linkedList, lruEntry);
    this.data[key] = new Buffer.from(JSON.stringify(value));
...
user.put('user3', {name:'admin', age: 32});
